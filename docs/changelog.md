...
- **Config hot-reload — `oduflow.toml` without a restart** — the running server can now reload its configuration in place, triggered by SIGHUP (`oduflow reload`, or `systemctl reload oduflow`). Provisioning a new `[team.X]`, or changing a quota, lifecycle window, or agent setting takes effect immediately and no longer disrupts other teams. The reload is **validate-before-apply**: an invalid config leaves the running server untouched (it keeps serving the previous config), and `oduflow reload --check` validates the file in-process (non-zero exit on error) so a config-management tool can gate a deploy on it. A valid config is swapped atomically and applied by re-running the idempotent startup reconcile (shared infra, per-team resources, agent containers, disk quotas); one-time migrations are not re-run. The auth layer reads settings dynamically (the OAuth provider re-registers team clients on reload) so a hot-added team's `auth_token` works without a restart. Settings read only at startup — `[server] host`/`port`/`allow_insecure_http`, `[routing] mode`, `[storage] data_dir`, `[database]` credentials/image, `[oauth] oauth_base_url` — still require a full restart, and the reload log says so. Removing a `[team.X]` section stops serving that team but never deletes its data. Config **delivery** stays with the operator (Salt/Ansible/GitOps): Oduflow is only the reload target. See [decision record 0036](https://github.com/oduist/oduflow/blob/main/specs/0036-config-hot-reload.md).
