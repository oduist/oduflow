...
| `export_module_translations` | ✓ | Export a module's `.pot`/`.po` with Odoo's own exporter, write it into the module's `i18n/`, and return a summary plus a one-time download URL |
| `translation_status` | ✓ | Compare a module's terms, its database translations and its committed `.po` files — surfaces `.po` files that Odoo imports as zero translations |
